...
The `tidyr` package will prove useful in data restructuring or "tidying". All of the packages in the `tidyverse`, like `ggplot2` and `dplyr`, as well as all sorts of community-contributed packages, are designed to work seamlessly with data in tidy format.

---

#### Tidying example

The "tidy" format of a data can depend on what you want to use the data for. To get us started in thinking about tidy data, we'll return to the `penguins` data. (For instructions on loading `penguins`, see [Getting Started](LINK-HERE).)
...
Let's take a look at how to move from untidy data to tidy data, and the reverse. First, you will need the package that contains the functions `pivot_wider()` and `pivot_longer()`. The package is called `tidyverse`, and you can install and load it like this:
...
Now you can learn how to restructure data. Begin with the first, tidy version:
...
(Note that _no_ values of `species` end in 0's, so the above code changes nothing. If a value of `species` ended in, for example, three "O's", the result would only retain one.)
