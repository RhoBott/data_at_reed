...
### More aesthetics -- basically how to show more variables in one plot -- how to make complex and comprehensible graphs

***why is this section important? how to show more variables in your graph without making it busy***
In addition to `x`, `y`, `color`, and `fill`, there are numerous geometries and additional arguments available for `ggplot2`. Below are some examples.

#### Aesthetic mappings

The aesthetics `shape` and `linetype` can be used to display a categorical variable, while `size` can show a continuous variable. [This site](https://www.datanovia.com/en/blog/ggplot-aes-how-to-assign-aesthetics-in-ggplot2/#group-and-line-type) contains helpful examples on how to use each of these aesthetics, and highlights cases where they might be particularly appropriate.
...
### More with ggplot2 -- again change title

#### Additional geoms and layers
...
Although some of the most frequently used geoms are described above, some other commonly used geoms include:
  ...
  - `geom_jitter()`: for dealing withn [overplotting](https://ggplot2.tidyverse.org/reference/geom_jitter.html).

You can also add more layers to your plots using the following functions:
