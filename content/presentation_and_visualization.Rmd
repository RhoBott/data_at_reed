...
Once you have a basic graph, you may want to customize it more to fit your needs. Starting with a scatterplot showing penguin bill length and body mass by island which was created above, adding a few simple commands allows you to adjust the graph to your liking. `labs()` is used to customize axis labels, as well as adding titles, subtitles, and captions. `theme()` is used to change the overall theme of the graph. Some options are `theme_dark()`, `theme_bw()` and `theme_classic()`. Using `scale_color_manual()`, the colors in the graph can be manually set. These are just a few ways to make a graph your own.
...
ggplot(data = penguins, 
  ...
  labs(x="Bill length (mm)", y="Body mass (g)", title="Palmer Penguins", subtitle="Examining penguin body mass and bill length")+
  theme_bw()
