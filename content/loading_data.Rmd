...
R is a powerful tool for data cleaning, analysis, and visualization. The first step to data work is loading your data. Below you can find instructions to bring data into R in a number of ways: using use a pre-loaded dataset, data from a .csv, Excel Sheet or a Google Sheet, or data found on the internet. 
...
When working with a package, there are a couple of options for finding the data contained in that package. (While all of these examples focus on the `palmerpenguins` data, you could use these instructions for any package of interest.)
...
Your data is loaded! You're ready to go. `r emo::ji("racing_car")`
...
The above code takes the file `cats.csv`, located in a folder `Bio123` within a `Reed` folder on the `Desktop`. The filepath `Desktop/Reed/Bio123` gives directions to `read_csv()` on where to find `cats.csv`. 

`read_csv()` not only finds this data, but also makes it into a dataset, which is then assigned to (using the `<-` operator) the name (`cat_data`). This data is now available for use in R (you can confirm this using `View()`).
...
The package `readxl` contains a function called `read_excel()`, which is very similar to `read_csv()` (see above). The main difference between these two tools is that the file is be a `.xls` or `.xlsx`, rather than `.csv`:
...
NOTE: Before you take data from a website, make sure you are allowed to scrape and analyze the data. (LINK-HERE)
...
2. The "selector" code that specifies where on the page the data table is located

If you want to learn more about `rvest`, [the Tidyverse documenetation for `rvest`](https://rvest.tidyverse.org/) contains a more extensive overview of the package and its functions.
