...
Depending on how datasets are included in a package, you might run that command and see nothing, or maybe just see `<Promise>` up in your Environment in the top-right corner of your RStudio. It might not look like your data is loaded, but it is. Try running the `View()` function in your console, which just takes in the name of your dataset.
...
`read_csv()` not only finds this data, it also makes it into a data frame, which we then save using the `<-` operator under a name of our choosing (`cat_data`). This data is now available to us to use in this R session. You can `View()` it, make a graph with it, or display it in a table.
...
Before you do take data from a website, make sure you are thinking about whether or not this is data that you are allowed to take and use for your own purposes. (ADD RESOURCES)
