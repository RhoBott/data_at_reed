...
Often times you find yourself with a data set that includes data in the form of a string. In many cases a string in a data set will be in the form of text! The package `stringr` is really useful tool for working with this type of data and thankfully is already included in the `tidyverse` package, so if you already installed, that you should be good to go!

Instead of going through a step-by-step tutorial on exaclty how each `stringr` function works, we'll go through a couple of concrete examples using the `penguins` dataset to show off different ways to use `stringr` in your data wrangling

Some important notation to understand are the following symbols:

1. ? : means 0 or 1
2. + : means 1 or more
3. * : means 0 or more
4. ^x : means 'begins with x'
5. x$ : means 'ends with x'
6. [xyz] : means 'includes x or y or z'

These will make a lot more sense as we dive into our first example:

In this first example we'll filter so that we only pull out species whose names end in 'p'. We can use the function `str_detect()` where we specify which variable we want to work with.

Here we're filtering our data to only look at penguin species that start with the letter 'A' who belong to an island that ends with the letter 'm'. We then use the function `str_to_upper()` to change our values for species to be all upper-case!
...
penguins %>%
  ...
  head()
```
Now if you're comfortable with `dplyr` you may be realizing that this is a bit of a silly way to solve this problem as we could very easily just use `filter(species == "Adelie" & island == "Dream)`, but if you were dealing with a data set that had text that wasn't split into factors, using `str_detect()` makes a lot of things possible.
...
Importantly this can be extended beyond just a letter, so that we can look for observations that start or end with any given combination of characters:
...
Next we'll take a look at an example where we want to only look at species that end in 1 or more "o's", and then we use a cool function called `str_to_upper' to capitalize the names of those species:
...
(of course theres also a corresponding `str_to_lower`). 

following off of our previous code we can then use a function called `str_remove()` to drop zero or more "O's" from the end of each one. You don't get to see it here but if some of our species ended with and number of "O's" our code would have removed all of those "O's" as well.

```{r}
penguins_edited %>%
  mutate(species = str_remove(species,"O*$")) %>%
  head()
```
