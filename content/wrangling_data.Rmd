...
Many of the previous examples have worked with numerica data; you may also work with character, or "string" data. String data can include numbers, letters, spaces, and special characters. Strings are most often represented as `"text surrounded in quotes"` in R. 

Package `stringr` is  useful tool for working with string data. Instead of going through a step-by-step tutorial for each `stringr` function, the below code presentsconcrete examples using the `penguins` dataset to demonstrate some of the capabilities of `stringr`.
...
The reader can the dimensions of the tibble (344 x 8), the variable names and their types, and the first ten rows. 

In contrast, printing out an entire `data.frame` is a bit unwieldy:
