...
When you first get a dataset, whether it is from a class, downloaded from the internet, or given to you by a professor, there might be ways that you want to change that data to make it easier to work with, add more information, or summarize your data. These all fall under the term "Data Wrangling". Let's learn how to wrangle our data, how to get more out of it, and how to keep it tidy.

### Tidy Data Principles, Reshaping Data, and `tidyr`

Packages in the `tidyverse`, like `ggplot2` and `dplyr`, are built to work with tidy data. The `tidyr` package can help us tidy up a dataset by rearranging how the data is structured.
...
Each variable has its own column, each observation (a penguin on a given island in a given year) is a row, and each value has its own cell.
...
This dataset technically contains the same data, but in this version, the mean body mass is held in three different columns, one for each year. Year and body mass are both variables that should have their own columns, but here they do not. If year is a variable that we have in our data, then there should be a column called "year", and the same is true for body mass.

Sometimes, the data you are asked to analyze might look more like the second, untidy version than the first, tidy version. We can move from tidy to untidy or vice versa using `tidyr`.

Let's start off with the first, tidy version:

```{r, message = FALSE, warning = FALSE}
penguins_sum
```

To make the untidy version, we "pivot" this data from a "long" format to a "wide" format. The `pivot_wider()` function from `tidyr` will help us to do that:
...
The `id_cols` (read: ID columns) are the variables that, together, identify what makes a row unique in the original data, `names_from` determines what variable from the old data will determine the names of columns in the new data, and `values_from` is the name of the variable in the old data that will be contained in cells in the new data.

Now, if we are given this untidy table and want to make it tidy, we can pivot from wider to longer using `pivot_longer()`.
...
If you are interested in learning more about tidy data and pivoting, we recommend the chapter in [R for Data Science](https://r4ds.had.co.nz/tidy-data.html).

### Transforming Data With `dplyr`

Now that our data is tidy we can use `dplyr` to further transform our data.

There are 6 fundamental `dpylr` functions that should allow you to perform the data wrangling that you find yourself needing to do. For the following examples we will be using the full `penguins` dataset.
...
Let's say that you want to take a subset of your full dataset, by only picking rows that meet certain criteria. For example, in our penguins dataset we could choose to look at female penguins only:
...
Or perhaps we want to only look at penguins with a bill length that is less than 40mm:
...
Let's `arrange()` our data by bill length, in ascending order:
...
We can also arrange by the same variable in descending order:
...
A dataset can have a lot of different variables, but sometimes we are only interested in a few. We can use the `select()` function focus on only those variables we are interested in. All you have to do is add all of the variables that you want inside of the function.
...
What if you wanted create a new column or "mutate" an existing column in some way? The `mutate()` function makes both of these things easy. For this example we will go back to using our `penguins_sum` data frame.

If we want to introduce a new column, we choose a name for it and then indicate how we want to calculate it. For example, we can create a new column for the mean body mass in kilograms instead of grams:
...
You might have noticed that the functions `group_by()` and `summarize()` were used in our explanation of tidy data with the data frame `penguins_sum`. Let's work step-by-step from our original dataset to `penguins_sum` to get a better feel for what that code was doing.
...
Notice how this code did not actually change the way our data frame looks. This is because the `group_by()` function changes the metadata, you can think of this as R getting the data frame ready for a further transformation to be applied to it. We can now apply the `summarize()` function to get what we want.
...
Now we have the summary statistic we wanted (mean body mass) for each island, in each year.

All that the `na.rm = TRUE` is doing is removing any missing values in `body_mass_g` so that we do not report missing values in our summary.
...
If you are comfortable with `dplyr`, you may be realizing that this is not the most efficient way to solve this problem as we could very easily use `filter(species == "Adelie" & island == "Dream")`, but if you were dealing with a dataset that had text that was not split into factors, using `str_detect()` makes a lot of things possible.
