...
Next, we'll take a look at an example where we want to only look at species that end in 1 or 2 "o's", and then we use a cool function called `str_to_upper()' to capitalize the names of those species:
...
The last function that we'll show off here will be `str_replace()`. In this example we'll replace the "e" at the end of "Biscoe" with "otti". 
...
penguins %>%
  ...
  head()
```
This is a rather silly example but there are plenty of situations in which making replacements such as these are important for a project you're working on.

If you want to do a more extensive replacing of patterns you can use `str_replace_all()`. Here we replaced all occurrences of vowels with "x".
