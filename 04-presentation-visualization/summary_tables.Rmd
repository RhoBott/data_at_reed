...
You can create a baseline table just by adding `gt()` onto the end of your code
...
We can also adjust some of our column names and include a title by adding onto our baseline table. Notice that we can use the `md()` wrapper which stands for "markdown" to use markdown styling in our tables.
...
As with kable there is so much that you can do to customize your `gt` tables. We've only scratched the surface here, but there are a lot of great resources that go in depth including [this one](https://gt.rstudio.com/reference/index.html).
