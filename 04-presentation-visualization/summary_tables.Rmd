...
Another approach for creating tables is the `gt` package. `gt` stands for "grammar of tables," and feels a lot like working with `ggplot`'s grammar or graphics.
...
You can create a baseline table just by adding `gt()` on to the end of your code:
...
We can also adjust some of our column names and include a title by adding on to our baseline table. Notice that we can use the `md()` wrapper, which stands for "markdown," to use markdown styling in our tables; the `**` on either side will bold the `title` argument.
