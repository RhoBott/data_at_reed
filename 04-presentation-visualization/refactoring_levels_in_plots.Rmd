...
When we want to order a categorical variable (`species`) by some quantitative variable (`n`) `fct_reorder()` is a great option. Generally the syntax is as follows:
...
With boxplots it's a little bit more tricky because, we don't necessarily have some explicit variable in our dataset that we can tell `fct_reorder` to use as a reference. Fortunately `fct_reorder` makes it straightforward to order boxplots by some function. Generally the function should be a summary function that can be applied to your y-variable. A few common examples include `max`, `min`, `median`, and `mean`.

Below we show a plot with multiple boxplots which be default gets ordered alphabetically, and then a plot with multiple boxplots ordered by the median.
...
To reorder by the median we can use the argument `.fun`. Note the inclusion of `na.rm = T`, sometimes when the data contains missing values, it will cause the reordering to not function properly. Note that we don't write the function as `median()`, but rather as `median`.
...
Here we create a vector of our levels put in the desired order, and then feed that vector into our `fct_relevel` call.
