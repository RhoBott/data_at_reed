...
The package `readxl` contains a function called `read_excel()`, which is very similar to [`read_csv()`](https://www.reed.edu/data-at-reed/resources/R/from-a-.csv.html). The main difference between these two tools is that the file is be a `.xls` or `.xlsx`, rather than `.csv`:
