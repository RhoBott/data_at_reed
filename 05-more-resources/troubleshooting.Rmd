...
Deciphering error messages in R, especially the first time you see them, can be a perplexing and time-consuming task. Here our team offers some tips and tricks to hopefully make your debugging experiences a bit less painful.

#### First steps

First, we recommend looking for errors in syntax; these are often small.

Start by checking your code for:

  - Misspellings: words, functions, or names of datasets and variables. (Note that objects in R are case-sensitive; `data` and `Data` are not interchangeable.)
  - Commas: missing or duplicate
  - Parentheses / brackets: missing or duplicate
  - Pipes `%>%`, used to connect code: do you have the connections you need?
  - Within `ggplot()` code: are you missing plus signs `+` (used to connect plotting code)
...
Sometimes, your R session can just get a bit backed up; in that case, saving your work and restarting RStudio may fix the problem. (Yes, "try turning it off and on again" _is_ sometimes the answer.)
...
You can always look up a function name on the internet and read its documentation, but most R packages and functions have help files that can be accessed through the R Console.
