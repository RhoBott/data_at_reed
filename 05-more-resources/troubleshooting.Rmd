...
Deciphering error messages in R, especially the first time you see them, can be a perplexing and time-consuming task. This page offers some tips and tricks compiled by the team to, hopefully, make your debugging experiences a bit less painful.

#### First steps

Below, we outline four different tips for approaching troubleshooting your code. That being said, we recommend starting with looking for small syntactical mistakes.

A good list of things to quickly look for are:

  - Misspelled words, functions, or names of datasets and variables---note that objects in R are case-sensitive
  - Missing commas
  - Unclosed and/or extra parentheses or brackets
  - Missing pipes `%>%` to connect code
  - Missing plus signs `+` to connect `ggplot()` code
  - All necessary packages and data loaded---errors will often look like "could not find function" or "object not found" in this case
  - Using the correct file path when trying to load in data or objects

Sometimes, your R session can just get a little bit backed up, so saving your work and restarting RStudio can fix the problem. The age-old "try turning it off and on again" method can sometimes be the answer. 
...
You can always look up a function name on the internet and read it's documentation, but most R packages and functions have help files that can be accessed through the R Console.
...
3. Add `print()` statements in your code. This can take a little bit of time to get used to, but the basic idea is that by adding something like `print("got to here!") in your code, you'll still get an error, but you'll be able to see if your code at least made it to that point successfully.
   
Alternatively, if you're creating objects in your code you can print those objects when they're created in the code to check if they're outputting what you expect.

4. Depending on what type of code you're writing, you can always try using `#` at the start of a line to comment out lines of your code. If your code runs successfully after you comment out a line, it's a good sign that the problem is happening there.
...
The resulting dataset shows `NA` values where our means should be. This happens because there are `NA`s in these columns. Even just one `NA` will get in the way of our calculations. Luckily, there is a very easy fix for this: using the argument `na.rm = TRUE`. This tells the `mean()` function to remove the `NA`s from its calculation. We can try the same summarization using `na.rm` like this:
