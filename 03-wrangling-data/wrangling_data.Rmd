...
Let's take a look at how to move from untidy data to tidy data, and the reverse. The functions that allow you to do this, `pivot_wider()` and `pivot_longer()`, exist in the package `tidyr`. This package is included in `tidyverse`, along with many other helpful packages, so you can install and load `tidyverse`:
